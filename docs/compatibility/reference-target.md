...
| U-44 | **`EnableCaseSensitiveItemIds`'s default, and that it is the server's rather than a library's.** It is declared `= true` `[source: MediaBrowser.Model/Configuration/ServerConfiguration.cs:89 @ v10.11.11]` and read as a server-wide setting `[source: Emby.Server.Implementations/Library/LibraryManager.cs:650 @ v10.11.11]`. This is [003's OQ-9](../../specs/003-library-configuration-and-scanning/spec.md#7-open-questions), which asked exactly this and had nothing to answer it with; the one server ever measured has the flag **set**, so it cannot answer either. A source reading is not a measurement | Defaults to case-**insensitive**, per library, frozen at creation — 003 §3.6's own decision, which never claimed to match a default. The identifiers differ from the reference's either way ([§1.4](behaviours.md#14-item-identifiers-are-32-lowercase-hex-characters)), but the **item count** does not: two files differing only in case are one item here and two there ~~, and it is one of the forty-seven differences 003 declares over its own fixture~~. **Struck 2026-09-05 by 003's T17, which wrote that declaration: it is not one of them, and cannot be.** The tree the recorded reading was taken over holds no case-only-differing pair — over the fifty-eight items [reference-fixture-reading.json](reference-fixture-reading.json) names in the three built libraries, no two differ only in capitalisation — so the difference is unobservable over this fixture **in either direction**, and building the pair would add one Atrium has and the reading has no row for. This row is therefore still owed a probe, which is what the last column already says | 003 plan §6.3, and 003 OQ-9's 2026-09-05 note | Yes — two files differing only in case, in one library, on a server whose setting has not been touched |
| U-45 | **What the reference calls a `Season` that carries no number.** [003 §3.7.2](../../specs/003-library-configuration-and-scanning/spec.md#37-sort-names) says a season's whole sort name is its four-digit number, which leaves nothing at all when there is no number; the source answers the raw name `[source: MediaBrowser.Controller/Entities/TV/Season.cs:151 @ v10.11.11]`. Nothing has ever sent the reference an unnumbered season and asked. The case is not hypothetical — the reference's own recorded reading of this project's fixture tree contains a `Season Unknown`, and [reference-fixture-reading.json](reference-fixture-reading.json) records four fields per item and no sort name, so the recording that exists cannot answer it either | Keeps the raw name, which is the source's reading and 003 §3.7.2's 2026-09-05 amendment. The alternative — the empty string — sorts every unnumbered season ahead of every numbered one in every series at once | 003 §3.7.2; 003 plan §6.6; asserted in `TestAMissingNumberContributesNoSegmentAtAll` | Yes — one series holding an episode whose filename names no season, read back with its sort name |
| U-46 | **The shape of an explicit sort title.** [003 §3.7.3](../../specs/003-library-configuration-and-scanning/spec.md#37-sort-names) says such a title is lowercased and digit-padded but not article-stripped, and the source says it uses **two** of §3.7.1's six steps and neither configured character list `[source: MediaBrowser.Controller/Entities/BaseItem.cs:535-536 @ v10.11.11]`. The library measured for [OQ-6](../../specs/003-library-configuration-and-scanning/spec.md#7-open-questions) contained almost no item carrying one, so nothing was observed running. This row is the *shape*; OQ-6 is the neighbouring question of whether §3.7.2's three types survive one | Implements the source's two steps for **all eight** kinds, the three overriding types included — which is AC-15 in terms | 003 §3.7.3; 003 plan §6.6; asserted in `TestAnExplicitSortTitleReplacesTheDerivationForEveryType` | Yes, but it needs a **library**: an item carrying a sort title, in each of the three overriding types and one that is not |
| U-47 | **The two `MediaType.Audio` extras tokens.** The reference recognises `theme` as a whole filename and `theme-music` as a directory name, gated on the file's media type `[source: Emby.Naming/Video/ExtraRuleResolver.cs:40-44 @ v10.11.11]`. No probe here has sent a file named for any extra | Does not implement either, so `theme.flac` under a `music` root is an ordinary track. Under `movies` and `tvshows` the tokens would change nothing, because §3.2's lists admit no audio extension and refuse such a file first. An accepted shortfall in the direction that shows **more** than the reference | 003 plan §6.1 | Yes — a `music` root holding `theme.flac` and a `theme-music/` directory holding one track |
| U-48 | **The numbering expressions 003 declines.** The reference's optimistic bare-number forms (`01.avi`, `01 - blah.avi`, `blah - 01.avi`, `Foo Bar 889`), `Episode 16` standing alone, the part and chapter forms, and the production year it takes out of an episode's filename `[source: Emby.Naming/Common/NamingOptions.cs:320-360 @ v10.11.11]`. Read from source; none has been sent to a running reference and none is in the fixture tree | Implements [003 §3.4](../../specs/003-library-configuration-and-scanning/spec.md#34-series-seasons-and-episodes)'s family and not the reference's whole list. Every one of them places **fewer** episodes here than there, and §3.8's unplaceable count is what shows an operator which | 003 plan §6.2; asserted as a set in `TestTheNumberingFamilyIsTheOneTheSpecificationNames` | Yes — one series directory holding a file of each declined shape |
| U-49 | **The reference's further conditions on a multi-disc album.** There a parent is a multi-disc album only when it holds no audio of its own and **every** music-holding subfolder is a disc folder; a stray track beside two disc folders makes the disc folders plain `Folder` items `[source: Emby.Server.Implementations/Library/Resolvers/Audio/MusicAlbumResolver.cs:128-139 @ v10.11.11]`. Read from source, never probed | Reads the disc rule off the directory's own name alone, so that tree stays one album. Fewer items here than there, the same direction as U-47 and U-48 | 003 plan §6.2 | Yes — an artist directory holding `CD1/`, `CD2/` and one loose track |
| U-50 | **What the reference does with two filenames that are canonically equal and byte-different.** Unicode has singleton mappings — U+212A KELVIN SIGN normalises to `K` — so a filesystem can hold two files whose names normalise to one key, in a **case-sensitive** library as well as an insensitive one. Nothing has asked the reference whether that is two items, one item, or an error | Fails that library's scan and removes nothing, naming the identifier and one of the two paths. The alternative — a last write winning — loses a file silently. macOS/APFS folds the pair into one file, so the case only arises on a filesystem that does not | 003 plan §6.3; 003 plan §7; asserted in `TestATreeThatDerivesOneIdentifierForTwoFilesFailsTheLibrarysScanAndRemovesNothing`, which **skips** where the filesystem folded the pair | Yes, and it needs a Linux filesystem: one directory holding `K.mkv` twice, spelled with the Kelvin sign and with the plain letter |
| U-51 | **What the reference does with a symbolic link a scan cannot follow.** A link to a directory, to a device, or to nothing at all is a shape [003 §3.2](../../specs/003-library-configuration-and-scanning/spec.md#32-what-is-considered-a-media-file) does not mention and no probe has sent | Follows the link and takes the **target's** size, and refuses what is not a regular file afterwards as a skipped path rather than as an error — because a dangling link, or a file moved between the directory read and the stat, must not cost an operator every item in the library | 003 plan §6.1's row, added at T8 | Yes — one root holding a link to a film, a link to a directory and a dangling link |
...
**U-45 to U-51 were added on 2026-09-05, at 003's closing audit, and they are the same collection
job 002's batch was.** Every one of them was already written down where it arose — in a `plan.md`
section, in a spec amendment, or in a doc comment beside the code that takes the decision — and none
of them was anywhere a differential run would look. Three things are worth saying about the batch
rather than leaving them to be counted.

**Five of the seven show *more* or *fewer* items than the reference, and the direction is not
uniform.** U-47 shows one more (a theme tune that is a track here); U-48 and U-49 show fewer (an
episode that is unplaceable here, an album that is one item here and three there). A run that only
counted would net them off. Each row therefore names the tree that isolates it, and the counting has
to be per-tree.

**Two of the seven cannot be settled by the fixture and need a tree and a filesystem of their own.**
U-50 needs a filesystem that does *not* fold two canonically-equal filenames together, which macOS
does and Linux does not — the assertion that carries it in this repository **skips** on a Mac and
runs on CI, which is the opposite of the usual asymmetry. U-46 needs a *library*, not a tree: an item
carrying an explicit sort title, which no path can supply and which 004 is what supplies.

**None of the seven is a divergence this project chose in order to be different.** Five are rules the
specification did not state and the code had to answer; two (U-47, U-49) are reference behaviour this
project declined to implement on a source reading alone, which is [AGENTS.md §1.3](../../AGENTS.md)
working as intended rather than a gap: nothing about what Atrium does was moved on source evidence,
and the reading is registered instead.
