...
| The L0 check stays green with no new rows | `conformance/` and `internal/httpapi`, ~~unchanged~~ **one literal added to each** | That this feature added no route |
...
**Amended 2026-09-05 at T18, which built the four rows and found the fourth one's word "unchanged"
was the wrong claim.** *"The L0 check stays green"* was written as though staying green were the
whole assertion, and staying green is what a check does when nothing asks it anything. **Both halves
of the L0 check derive what they expect from `surface.yaml`, so a route registered *together with a
row in that document* satisfies both by construction** — which is precisely the change a feature
makes when it decides to grow a route, and precisely the change §10 refuses. Measured: registering
a `POST /Library/Refresh` and declaring it in both copies of the surface document leaves
...
`[measurement: 003 T18, 18 mutations, 2026-09-05]`. So each half gains a **literal** — the eleven
rows of 001 and 002, written out — and a reviewer who means to add a route has to edit it in a file
that says why they should not. Three other checks fail on that build for reasons of their own (the
wire sweep's route coverage, the response-model declaration and the embedded table's row count) and
not one of them is a statement about *this feature* having added a route.

**And one control this task wrote was relative where it had to be absolute**, which is the same
finding T15 and T16 both recorded one shape along. AC-2's *"a second scan of an unchanged tree
reports no changes"* needs a control that the second scan looked at anything, and comparing its file
counts against the **first scan's** is satisfied by a build that reports nothing examined for every
scan — both readings zero, both equal. It was the only survivor of this task's first mutation run,
and the comparison is now against the declared counts of the fixture rather than against the
previous run.
...
**Amended 2026-09-05 at T20, the closing audit, which ran thirty-two mutations across the sixteen
criteria and the four narrowings and found two rows of this table proven a layer below what they
say** `[measurement: 003 T20, 32 mutations, 2026-09-05]`.

- **Row 13 was the guess and the guess was right, which is the less interesting half.** AC-13's
  assertions were `library`'s and `internal/store/sqlite`'s, and the store's expected keys are
  computed by calling `library.SortKeyFor` — so a build that leaves the derivation correct and
  stores something else agrees with every one of them. Three lines in `Scanner.Scan` that set each
  item's sort key to its **name** left the whole repository green. Row 13 is now `library` + `app`:
  a scan of the fixture, compared against thirty-two keys **written down** in a file a guard fails
  if it ever names the derivation. The paragraph below this one, which said AC-13 *"is discharged
  against the stored key"*, was the sentence that made the gap invisible — it is struck there.
- **Row 4's store half asserted the read and not the write.** `TestAMultiPartFilmRoundTripsAsOneItem
  WithItsPartsInOrdinalOrder` empties `item_files` and re-inserts both parts by hand before
  counting them, so its *"one item row, two file rows"* counted the test's own inserts. A store that
  wrote only the first file of an item was caught by a transaction test one file along and by three
  of AC-14's, and by nothing about AC-4. The counts are asserted before the scramble now.
- **The other fourteen criteria each died on a behavioural mutation of production code**, and three
  of the sixteen needed more than one to separate the halves their own wording names (AC-6's
  companion, AC-15's two directions, AC-16's three). The four narrowings 002 T22's diagnosis points
  at each have a test **of the narrower claim**: §3.2's empty marker and its bounded search, §3.3's
  part-marker vocabulary, §3.5's leading-number tie-break, and §6.1's *"files being written"* as a
  property of a pair of scans. T20's entry in `tasks.md` carries the table.
...
reading — which is as close as it gets. ~~AC-13 is discharged against the **stored key**~~ — **it was
not, and this sentence is why nobody looked: struck 2026-09-05 at T20**, which measured that every
AC-13 assertion in the repository was about the derivation and that a scan storing each item's name
instead was green everywhere. It *is* discharged against the stored key now, over the fixture and
against literals. The step it still cannot take is that the ordering a client receives is that key's;
that step is in §8.3's table and is 005's.
