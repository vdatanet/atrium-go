...
| **Policy flags stored but unenforced** ([002 §3.5](../../specs/002-authentication-users-and-sessions/spec.md)) | A restriction that does not restrict | ~~All of them gate features v1 lacks~~ **Twenty-six of the twenty-eight do**; each is enforced in the change that adds its feature. *Corrected 2026-09-03, in the change that wrote the login path — which is the code that decides not to enforce them. `EnableRemoteAccess` and `AccessSchedules` are enforced by the reference **at authentication itself**: a user without remote access authenticating from outside the local network, and a user outside their permitted schedule, are each refused there `[source: Jellyfin.Server.Implementations/Users/UserManager.cs:595-611 @ v10.11.11]`. The feature they gate is not Live TV or sync, it is logging in, which v1 has — so v1 admits two logins the reference refuses, and the closing mechanism for these two is not "the feature arrives" but a measurement. Both refusals are unmeasured, each needing an account restricted that way and a caller outside the local network, and they are [U-15](reference-target.md) in the register. [002 §3.5](../../specs/002-authentication-users-and-sessions/spec.md) carries the same correction, amended on the same date, and says this row was owed it* |
